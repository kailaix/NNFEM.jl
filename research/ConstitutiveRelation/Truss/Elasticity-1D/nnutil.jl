...
function post_nn(ε::Float64, ε0::Float64, σ0::Float64, Δt::Float64)
    ...
    elseif nntype=="ae_scaled"
        #use only the first component
        f = x -> nnae_scaled(reshape([x/strain_scale],1,1))[1,1]*stress_scale
        df = ForwardDiff.derivative(f, ε)
    elseif nntype=="ae_scaled2"
        f = x -> nnae_scaled2(reshape([x/strain_scale;ε0/strain_scale;σ0/stress_scale],1,3))[1,1]*stress_scale
        df = ForwardDiff.derivative(f, ε)
    else
        error("$(nntype) must be specified.")
    end

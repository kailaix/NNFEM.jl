export DynamicMatLawLoss,DynamicMatLawLossTest, preprocessingTest, BFGS, preprocessing, ADAM, NNMatLaw
...
function DynamicMatLawLossTest(domain::Domain, globdat::GlobalData, state_history::Array{Any}, fext_history::Array{Any}, nn::Function, Δt::Float64)
    # todo convert to E_all, Ftot
    domain.history["state"] = state_history
    F_tot, E_all, w∂E∂u_all,σ_all = preprocessingTest(domain, globdat, hcat(fext_history...), Δt)
    DynamicMatLawLoss(domain, E_all, w∂E∂u_all, F_tot, σ_all, nn)
end
...
# only works in 1D
function preprocessingTest(domain::Domain, globdat::GlobalData, F_ext::Array{Float64},Δt::Float64)
    local Vi
    U = hcat(domain.history["state"]...)
    V = hcat(domain.history["stress"]...)
    # @info " U ", size(U),  U'
    M = globdat.M
    MID = globdat.MID 
    ...
    # # DEBUG
    # fext = hcat(domain.history["fint"]...)
    return F_tot'|>Array, E_all, w∂E∂u_all, σ_all
end
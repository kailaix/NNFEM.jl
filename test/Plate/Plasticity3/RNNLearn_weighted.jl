...
function compute_sequence_loss(strain_seq, stress_seq, nn)
    ...
    for it = 2:nt
        ...
        loss += mean((ys - y)^2) * ((ratio*nt-2)/(nt-2) - (ratio-1)/(nt-2)*it)

        # ratio = 2.0
        # if it > nt/2 
        #     ratio = 1.0
        # end
        # loss += mean((ys - y)^2) * ratio
    end
    return loss
end
#####################################################################################
...
#ADCME.load(sess, "Data/order$porder/learned_rnn_weighted_$(force_scale)_$(fiber_size).mat")

for i = 1:10
    BFGS!(sess, loss, 1000)
    ADCME.save(sess, "Data/order$porder/learned_rnn_weighted$(ratio)_$(force_scale)_$(fiber_size).mat")
end

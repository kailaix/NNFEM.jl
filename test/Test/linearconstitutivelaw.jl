...
@assert n==1
v = rand(1,9)
A0, B0, C0 = linear_constitutive_law(input, θ, nothing, true, false)
g = rand(1, 3)
γs = [1.,1e-1,1e-2,1e-3,1e-4]
L = zeros(5); Δ = zeros(5); δ = zeros(5)
for i = 1:5
    A, B, C = linear_constitutive_law(γs[i]*v +input,  θ)
    L[i] = sum(A.*g)
    Δ[i] = L[i] - sum(A0.*g)
